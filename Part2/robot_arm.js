...
var shapes = {
    ...
    finger: { points: [], colors: [], start: 0, size: 0, type: 0 },
};
...
for (var i = 0; i < fingerLookups.length; i++) {
    shapes.finger.points.push(fingerVerts[fingerLookups[i]]);
    shapes.finger.colors.push(white); // Assign white color for fingers
}
...
window.onload = function init() {
    ...
    loadShape(shapes.finger, gl.LINE_STRIP);
...
function render() {
    ...
    var armShape = solidMode ? shapes.solidCube : shapes.wireCube;
    var matStack = [];
    ...
    // Finger 1
    mv = mult(mv, translate(0.8, 0.0, -0.4));
    mv = mult(mv, rotate(elbow2, vec3(0, 0, 1)));
    mv = mult(mv, translate(0.5, 0.0, 0.0));
    matStack.push(mv);
    mv = mult(mv, scale(0.4, 0.15, 0.2));
    mv = mult(mv, rotate(fingerRotationX, vec3(1, 0, 0)));
    mv = mult(mv, rotate(fingerRotationY, vec3(0, 1, 0)));
    gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
    gl.drawArrays(armShape.type, armShape.start, armShape.size);
    mv = matStack.pop();
    
    mv = mult(mv, translate(0.15, 0.0, 0.0));
    ...
    // Finger2
    mv = mult(mv, translate(-0.6, -0.3, 0.35));
    mv = mult(mv, rotate(elbow2, vec3(0, 0, 1)));
    mv = mult(mv, translate(0.3, 0.0, 0.0));
    matStack.push(mv);
    mv = mult(mv, scale(0.4, 0.15, 0.2));
    ...
    // Finger3
    mv = mult(mv, translate(-1.1, -0.85, 0.4));
    mv = mult(mv, rotate(elbow2, vec3(0, 0, 1)));
    mv = mult(mv, translate(1, 0.0, 0.0));
    matStack.push(mv);
    mv = mult(mv, scale(0.4, 0.15, 0.2));
    ...
    mv = mult(mv, translate(-0.3, 0.5, 0.0));
    ...
    // Thumb
    mv = mult(mv, translate(-1.08, -0.1, -0.3));
    ...
    mv = mult(mv, translate(-0.3, -0.55, 0.0));
...
document.onkeydown = function handleKeyDown(event) {
    ...
    // Add Key Handling for Fingers and Thumb
    if (key == 'F') {
        if (thumbAngle < 45) thumbAngle += 5;
    } else if (key == 'f') {
        if (thumbAngle > -45) thumbAngle -= 5;
    }
    ...
    for (var i = 0; i < 4; i++) {
        if (key == 'A') {
            if (fingerAngles[i] < 45) fingerAngles[i] += 5;
        } else if (key == 'a') {
            if (fingerAngles[i] > -45) fingerAngles[i] -= 5;
        }
    }
...
function handleKeys(timePassed)
{
    ...
    if (isPressed("S")) s = Math.min(200, s + 10);

    //  D/d: Decrease movement speed
    if (isPressed("D")) s = Math.max(10, s - 10);

    //  Z/z: Sequential fingers closing right to left
    if (isPressed("Z")) {
        if (thumb > -45) thumb -= d;
        if (finger1 > 0) finger1 -= d;
        if (finger2 > 0) finger2 -= d;
        if (finger3 > 0) finger3 -= d;
        
    }

    //  G/g: Sequential fingers opening right to left
    if (isPressed("G")) {
        if (finger1 < 45) finger1 += d;
        if (finger2 < 45) finger2 += d;
        if (finger3 < 45) finger3 += d;
        if (thumb < 0) thumb += d;
    }
